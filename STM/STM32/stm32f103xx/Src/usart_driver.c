...
//	}
	...
	if (WordLength == USART_WORDLENGTH_9B)
		Instance->CR1 |= (1 << 12); // M = 1: 9 bits
	else
		Instance->CR1 &= ~(1 << 12); // M = 0: 8 bits

	// Cấu hình Parity
	if (Parity == USART_PARITY_NONE) {
		Instance->CR1 &= ~(1 << 10); // Tắt parity
	} else {
		Instance->CR1 |= (1 << 10); // Bật parity
		if (Parity == USART_PARITY_ODD)
			Instance->CR1 |= (1 << 9); // ODD
		else
			Instance->CR1 &= ~(1 << 9); // EVEN
	}

	// Cấu hình Stop bits
	Instance->CR2 &= ~(3 << 12);         // Xoá trước
	Instance->CR2 |= (StopBits << 12);   // Gán Stop bits
	...
	if (HWFlowControl == USART_HW_RTS)
		Instance->CR3 |= (1 << 8); // RTS enable
	else if (HWFlowControl == USART_HW_CTS)
		Instance->CR3 |= (1 << 9); // CTS enable
	else if (HWFlowControl == USART_HW_RTS_CTS)
		Instance->CR3 |= (1 << 8) | (1 << 9);

	// Bật USART
	Instance->CR1 |= (1 << 13); // UE = 1
